...
#  파이썬 문자열의 중요한 특징
# =========================================================
# 1. 문자열은 (Immutable) 이다 -> (불변) (문자열 연산 시 새로운 문자열이 만들어짐)
# 2. 문자열은 (Iterable) 하다 -> (반복가능) (반복처리 가능)
# 3. 문자열은 (Sequence)가 있다 -> (순서가 있음) (인덱싱, 슬라이싱 가능)
...
print(type(a[0]))                          # 문자열의 첫 글자 P 출력

b = [10, 20, 30]
b[0] = 40                          # 리스트 내용 수정이 가능할까?
print(b)                          # 리스트의 첫 번째 요소 40 출력

# 문자열 메소드로 변경해도 원본은 그대로, 새로운 문자열 객체가 생성됨
b = a.upper()

print(a, b)
print(id(a), id(b))                             # 다른 객체인가?
...
print(id(a[0]), id(a[0]))                   # 같은 객체인가?

# 슬라이싱 (start:end-1:step)
print(a[0:2])                       # Py 출력
print(a[:2])                        # Py 출력 (처음부터)
print(a[2:6])                        # thon 출력 (끝까지)
print(a[2:])                       # th 출력 -> 슬라이싱도 할 때마다 새로운 문자열이 생성됨
print(a[2:4])                       # th 출력 (2칸씩)
...
s = "1301"
print(f"{s[0]}학년 {s[1]}반 {s[-1]}번")                  # ✅ "1학년 3반 1번" 출력
